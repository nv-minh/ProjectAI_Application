...
def build_classifier_maxent(content, label):
    clf = linear_model.LogisticRegression(max_iter=10000).fit(content, label)
    return clf
...
def predicting_decisiontree(content, content_vec, clf):
    label_predict = clf.predict(content_vec)
    for i in range(len(content)):
        if content[i][:2] in [u'qc', u'tb']:
            label_predict[i] = 1
...
def predicting_knn(content, content_vec, clf):
    label_predict = clf.predict(content_vec)
    for i in range(len(content)):
        if content[i][:2] in [u'qc', u'tb']:
            label_predict[i] = 1
...
def predicting_maxent(content, content_vec, clf):
    label_predict = clf.predict(content_vec)
    for i in range(len(content)):
        if content[i][:2] in [u'qc', u'tb']:
            label_predict[i] = 1
...
def predicting_baseline(content):
    label_predict = np.zeros(len(content))
    for i in range(len(content)):
        if content[i][:2] in [u'qc', u'tb']:
            label_predict[i] = 1
    return label_predict
...
def classification(content_test, content_vec_train, content_vec_test, label_train, label_test, classifier, vectorize_method):
    if classifier == 'nb':
        ...
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\tfidf_nb_clf.pkl', 'wb'))
        matrix = classifying_nb(content_test, content_vec_test, clf, label_test)
        ...
        clf = build_classifier_decisiontree(content_vec_train, label_train)
        # if vectorize_method == 'bow':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\bow_dt_clf.pkl', 'wb'))
        # if vectorize_method == 'tfidf':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\tfidf_dt_clf.pkl', 'wb'))
        matrix = classifying_decisiontree(content_test, content_vec_test, clf, label_test)

    elif classifier == 'knn':
        clf = build_classifier_knn(content_vec_train, label_train)
        # if vectorize_method == 'bow':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\bow_knn_clf.pkl', 'wb'))
        # if vectorize_method == 'tfidf':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\tfidf_knn_clf.pkl', 'wb'))
        matrix = classifying_knn(content_test, content_vec_test, clf, label_test)

    elif classifier == 'maxent':
        clf = build_classifier_maxent(content_vec_train, label_train)
        # if vectorize_method == 'bow':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\bow_maxent_clf.pkl', 'wb'))
        # if vectorize_method == 'tfidf':
        #     pickle.dump(clf, open(r'C:\KhoiNXM\Spam message Vietnamese\Dev\vietnamese-spam-sms-filtering\models\tfidf_maxent_clf.pkl', 'wb'))
        matrix = classifying_maxent(content_test, content_vec_test, clf, label_test)
